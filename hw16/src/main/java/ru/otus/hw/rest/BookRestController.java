...
import ru.otus.hw.indicator.EntityNotFoundHealthIndicator;
...
public class BookRestController {
    ...
    private final EntityNotFoundHealthIndicator entityNotFoundHealthIndicator;

    public BookRestController(BookService bookService,
                              EntityNotFoundHealthIndicator entityNotFoundHealthIndicator) {
        this.bookService = bookService;
        this.entityNotFoundHealthIndicator = entityNotFoundHealthIndicator;
    }
    ...
    public BookDto updateBookById(@Valid @RequestBody BookModifyDto bookModifyDto, BindingResult bindingResult) {
        ...
        try {
            Book book = bookService.update(bookModifyDto.getId(),
                    ...
                    bookModifyDto.getAuthorId(),
                    bookModifyDto.getGenreId());
            entityNotFoundHealthIndicator.setHasError(false);
            return BookDto.toDt0(book);
        } catch (EntityNotFoundException ex) {
            checkExceptionAndAddFieldErr(ex, bindingResult);
            entityNotFoundHealthIndicator.setHasError(true);
            return createErrorDto(bindingResult);
        }
    }
