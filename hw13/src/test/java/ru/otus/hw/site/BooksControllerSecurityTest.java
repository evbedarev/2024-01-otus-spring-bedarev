...
public class BooksControllerSecurityTest {
    ...
    @DisplayName("Проверяет доступные эндпоинты для роли ADMIN")
    @ParameterizedTest
    ...
    @DisplayName("Проверяет эндпоинты для роли админа не доступны пользователю")
    @ParameterizedTest
    @MethodSource("getAdminUris")
    void shouldFailForUserWhenGetAdminPages(String uri) throws Exception {
        mvc.perform(get(uri)).andExpect(status().is4xxClientError());
    }
    ...
    @DisplayName("Проверяет эндпоинты для роли USER доступны пользователю user")
    @ParameterizedTest
    @MethodSource("getUserUris")
    void shouldCorrectGetUserPages(String uri) throws Exception {
        mvc.perform(get(uri)).andExpect(status().isOk());
    }
    ...
    private static List<String> getAdminUris() {
        return Arrays.asList("/edit/1","/delete/1");
    }

    private static List<String> getUserUris() {
